...
<body>
    ...
    <form method="post" action="q3.php">
        ...
        <label>
            ...
            <input type="date" name="date_fin" placeholder="Date de fin" required> Format des dates : "jour-mois-année",tout en nombre
        </label>
        <BR>
        ...
        if (isset($_POST['submit']) && $_POST['submit'] == 'Soumettre') {
            try {
                ...
                */
                ...
                if($_POST['exception']!=''){
                    ...
                    foreach($lines as $line){
                        ...
                        if(!preg_match($regex_exception, $line)){
                            $bdd->rollBack();
                            die("<P>Format d'exception incorrect, <BR> insertion annulée, recommencez tout</P>");
                            return;
                        }
                ...
                echo " - Erreur: aucune ligne insérée pour exception </p><BR>" ;
                if ($e->getCode()==23000){
                    echo "le nom du service existe déjà <BR>";
                }
                else{    
                     echo $e->getMessage(); 
                     // il faudrait tester tous les SQLSTATE d'erreurs
                     // pour éviter de montrer ce msg avec des noms d'objets de la db
